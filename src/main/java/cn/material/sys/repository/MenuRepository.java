...
import cn.material.sys.entity.Menu;
import cn.material.sys.vo.MenuNode;
import cn.material.sys.vo.SidebarNode;
import cn.material.sys.vo.ZtreeNode;
...
public interface MenuRepository extends BaseRepository<Menu, String> {
     ...
     * @return
     */
    @Query(value="select new cn.material.sys.vo.MenuNode(m.id, m.menuName,m.parent.id,m.serialNo,m.menuType) from Menu m where m.parent.id=:parentId order by m.serialNo asc")
     ...
     * @return
     */
    @Query(value="select distinct new cn.material.sys.vo.MenuNode(m.id, m.menuName,m.parent.id,m.serialNo) from Menu m order by m.serialNo asc")
     ...
     */
    @Query(value="select distinct new cn.material.sys.vo.ZtreeNode(m.id, m.parent.id, m.menuName) from Menu m where m.id !='root' order by m.serialNo asc")
     ...
     * @return
     */
    @Query(value="select new cn.material.sys.vo.SidebarNode(m.id, m.menuName, m.menuUrl, m.menuIcon, m.serialNo, m.parent.id) from Menu m where m.id=:menuId")
